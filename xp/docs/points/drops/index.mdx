...
The **name** is only visible to you and is useful to organise your drops. The name will be displayed to _you_ when you preview the content where the snippet was placed.
...
To get the code snippet of the drop, click on its name from the _Settings > Points > Drops_ page. This will open a popup showing a snippet looking something like `[xpdrop secret=abc123 id=123]`. Copy the snippet to your clipboard.
...
Did you know that you can wrap the snippet within the shortcode `[xpiflevel]` to conditionally award the drop points? The example code below will only trigger the drop if the learner's level is below 5. [Read more](/how-to/use-shortcodes/index.mdx#xpiflevel)
...
This section is only relevant to drops and backups created on **versions prior to XP+ 1.16.**

:::

Before XP+ 1.16, drops could not be backed up and restored on the same site. In short, this was due to the lack of unique identifier in the drop snippet. Since XP+ 1.6, the drop shortcode includes both the drop secret, and the drop ID. This allows for backups to be restored on the same site.

Note that all shortcodes should be updated to the new shortcode including both secret and ID for their restoration to work.
