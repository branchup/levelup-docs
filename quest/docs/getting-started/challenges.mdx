...
Challenges are made of one or more objectives, potentially leading to a reward. They differ from [quests](./quests/) in that they do not include a narrative. Moreover, challenges can be limited in time, and be automatically recurring.
...
Challenges can trigger recruitment automations when they are completed. This means that when a challenge is completed (or failed), the recruit can be automatically recruited for another quest, achievement or challenge.

For example, we could respond to challenges that were not completed on time by giving recruits a second chance to complete them, perhaps with a moderate reward. Alternatively, we could send them on a remediation quest path that they would otherwise not have had access to.